...
public:
	...
	inline T& operator[](int insx) noexcept;
...
void TVector<T>::insert(int pos, T number) {
	if (pos <= _size && pos >= 0) {
		...
		throw std::logic_error("Error in insert: position out of range");
}

template<class T>
void TVector<T>::erase(int pos, int count) {
	if (pos + count > _size || pos < 0 || count <= 0) {
		throw std::logic_error("Error in erase: position or count out of range");
	}
